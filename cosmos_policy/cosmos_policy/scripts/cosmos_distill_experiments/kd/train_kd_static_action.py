...
"""
Identical to train_kd_static.py / train_kd_static_av.py in every respect (same StaticTrainParams
schema, same DistillShardDataset, same resume/checkpoint/logging shape) except the loss: this uses
batch_prep.combined_kd_loss_action_only instead of combined_kd_loss/combined_kd_loss_action_value_only,
so ONLY the action-chunk slot of the shared video latent is supervised -- both the future-state
slots AND the value slot are excluded from the loss entirely (narrower than train_kd_static_av.py,
which still supervises value). See combined_kd_loss_action_only's own docstring for why this is the
only lever available here (same reasoning as train_kd_static_av.py's own docstring: the static
dataset's xt/sigma were already noised at build time with every slot as a denoising target,
rebuilding it is out of scope for this variant).
...
positions in the real teacher/student's fixed grid. Only the loss stops supervising every slot but
the action one.
...
action-only loss term -- same two-term total loss as train_kd_static.py's own module docstring,
just narrowed to action-only like the real term here.
...
def train(params: kd_params.StaticTrainParams) -> None:
    ...
    action_latent_idx, _ = compute_action_value_latent_idx()
    ...
    for iteration in itertools.count():
        ...
        loss, distill_loss, ground_truth_loss = combined_kd_loss_action_only(
            student_x0.float(),
        ...
        if synthetic_dataset is not None:
            ...
            _, synthetic_distill_loss, _ = combined_kd_loss_action_only(

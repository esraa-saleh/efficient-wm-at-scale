...
"""
Identical to train_kd.py in every respect (same KDParams schema, same teacher/student device
placement, same LIBERODataset/DataLoader, same resume/checkpoint/logging shape) except the loss:
this uses batch_prep.combined_kd_loss_action_value_only instead of batch_prep.combined_kd_loss, so
only the action-chunk and value slots of the shared video latent are supervised -- the future-state
slots (current/future proprio, wrist image, third-person image) are excluded from the loss
entirely, not just down-weighted (see combined_kd_loss_action_value_only's own docstring).

Unlike train_kd_static_av.py, this path sees a real `data_batch` fresh off LIBERODataset every
iteration, which already carries the real `action_latent_idx`/`value_latent_idx` for that batch
(LIBERODataset.__getitem__ sets them directly) -- so, unlike the static variant (which has no way
to check its own derived constants against anything), this asserts those match
batch_prep.compute_action_value_latent_idx()'s derivation every iteration. That's what makes
train_kd_static_av.py's use of the same derived constants trustworthy: if this assert ever fired,
it would mean DATASET_KWARGS and compute_action_value_latent_idx() have drifted apart, which would
otherwise corrupt train_kd_static_av.py silently.
...
every step, only the loss stops supervising the future-state ones.

If `params.synthetic_dataset_dir` is set, a SECOND, always-distill-only action/value-only loss term
is added -- same two-term total loss as train_kd.py's own module docstring, just narrowed to
action+value like the real term here. No teacher call needed for it -- see train_kd.py's own
docstring for why.

Usage:
    python -m cosmos_policy.scripts.cosmos_distill_experiments.kd.train_kd_av --kd_params <path/to/kd_params.yaml>
...
def train(params: kd_params.KDParams) -> None:
    ...
    action_latent_idx, value_latent_idx = batch_prep.compute_action_value_latent_idx()
    ...
    for iteration, data_batch in enumerate(itertools.cycle(loader), start=0):
        ...
        # LIBERODataset-assigned indices -- see this module's own docstring for why that matters.
        ...
        assert torch.all(data_batch["value_latent_idx"] == value_latent_idx), (
            f"data_batch['value_latent_idx'] does not match compute_action_value_latent_idx()'s "
            f"derived {value_latent_idx} -- DATASET_KWARGS and that derivation have drifted apart."
        )
        ...
        loss, distill_loss, ground_truth_loss = batch_prep.combined_kd_loss_action_value_only(
            student_x0.float(),
        ...
        if synthetic_dataset is not None:
            ...
            _, synthetic_distill_loss, _ = batch_prep.combined_kd_loss_action_value_only(
